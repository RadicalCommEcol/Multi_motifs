...
for (week_i in unique(fitness2$Week)){
  ...
  #fitness_week_i <- fitness_week_i %>% group_by(Plot,Subplot,Plant,ID,num.plantas,Fruit,Seed) %>%
  #  count(wt=Visits) %>% rename(Visits_tot = n)
  
  #fitness_week_i$Subplot_Plant_Label <- paste(fitness_week_i$Subplot,fitness_week_i$Plant,sep = " ")
  #fitness_week_i <- fitness_week_i %>% mutate(Seeds_tot = num.plantas*Seed)
  
  #Nuevos fitness maria
  fitness_week_i <- fitness_week_i %>% group_by(Plot,Subplot,Plant,ID) %>%
    count(wt=Visits) %>% rename(Visits_tot = n)
...
write_csv(visit_list, "Processed_data/Motifs_WEEK/2020_Caracoles_WEEK_SPECIES.csv")
