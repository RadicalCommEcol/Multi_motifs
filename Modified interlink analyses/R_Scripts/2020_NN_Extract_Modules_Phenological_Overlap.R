...
for (Plot_i in 1:9){
  ...
  #################################################
  intralinks <- S_edge_list %>% filter(layer_from == layer_to)
  interlinks <- S_edge_list %>% filter(layer_from != layer_to)
  
  # We make the weight of interlinks proportional to the number of visits 
  # shared by the two species in proportion to the total of visits received by the plants
  
  if(nrow(interlinks)>0){
    
    interlinks_zero <- interlinks # Weighth before and after the correction
    interlinks_zero$weight2 <- NA
    interlinks_zero$Plot <- Plot_i
   
    for(i.inter in 1: nrow(interlinks)){
      
      str_poll_from <- pollinator_strength$strength[(pollinator_strength$layer_from ==
                                                       interlinks[i.inter,1] %>% pull()) &
                                                      (pollinator_strength$node_to ==
                                                         interlinks[i.inter,2] %>% pull())]
      
      str_poll_to <- pollinator_strength$strength[(pollinator_strength$layer_from ==
                                                     interlinks[i.inter,3] %>% pull()) &
                                                    (pollinator_strength$node_to ==
                                                       interlinks[i.inter,4] %>% pull())]
      
      interlinks$weight[i.inter] <- interlinks$weight[i.inter]* str_poll_from / max(str_poll_from,str_poll_to)
      interlinks_zero$weight2[i.inter] <- str_poll_from / max(str_poll_from,str_poll_to)
      
    }
    
    S_edge_list <- bind_rows(intralinks,interlinks)
    
    interlinks_list <- bind_rows(interlinks_list,interlinks_zero)
...
interlinks_list_real <-interlinks_list
interlinks_list_real$layer_from[interlinks_list_real$layer_from == "BEMA"] <- "B. macrocarpa"
interlinks_list_real$layer_from[interlinks_list_real$layer_from == "CETE"] <- "C. tenuiflorum"
interlinks_list_real$layer_from[interlinks_list_real$layer_from == "CHFU"] <- "C. fuscatum"
interlinks_list_real$layer_from[interlinks_list_real$layer_from == "CHMI"] <- "C. mixtum"
interlinks_list_real$layer_from[interlinks_list_real$layer_from == "LEMA"] <- "L. maroccanus"
interlinks_list_real$layer_from[interlinks_list_real$layer_from == "MESU"] <- "M. sulcatus"
interlinks_list_real$layer_from[interlinks_list_real$layer_from == "PUPA"] <- "P. paludosa"
interlinks_list_real$layer_from[interlinks_list_real$layer_from == "SCLA"] <- "S. laciniata"
interlinks_list_real$layer_from[interlinks_list_real$layer_from == "SOAS"] <- "S. asper"
interlinks_list_real$layer_from[interlinks_list_real$layer_from == "SPRU"] <- "S. rubra"
...
G_F_list <- G_F_list %>% rename(node_from = ID)

interlinks_list_real <- interlinks_list_real %>% dplyr::left_join(G_F_list,
                                                                  by = "node_from")

interlinks_list_real$G_F[interlinks_list_real$G_F=="Flower_beetles"] <- "Flower beetles"
interlinks_list_real$G_F[interlinks_list_real$G_F=="House_flies"] <- "House flies"
interlinks_list_real$G_F[interlinks_list_real$G_F=="Small_beetles"] <- "Small beetles"
interlinks_list_real$G_F[interlinks_list_real$G_F=="Small_flies"] <- "Small flies"
interlinks_list_real$G_F[interlinks_list_real$G_F=="Solitary_bees"] <- "Solitary bees"
interlinks_list_real$G_F[interlinks_list_real$G_F=="Wasp"] <- "Wasps"
interlinks_list_real$G_F[interlinks_list_real$G_F=="Big_beetles"] <- "Big beetles"

library(RColorBrewer)
library(patchwork)
p1 <- ggplot(interlinks_list_real,aes(x=layer_from, y = weight))+
  geom_boxplot(show.legend = FALSE)+
  geom_point(aes(color = as.factor(G_F)),alpha = 1,position = "jitter")+
  scale_color_brewer(palette = "Paired")+
  #facet_wrap(~ Plot)+
  labs(y="Phenological weight of interlinks",x=NULL,color = NULL)+
  guides(color = FALSE)+
  coord_flip()+
  theme_bw()+
  theme(axis.text.y = element_text(face = "italic"))

p2 <- ggplot(interlinks_list_real,aes(x=layer_from, y = weight*weight2))+
  geom_boxplot(show.legend = FALSE)+
  geom_point(aes(color = as.factor(G_F)),alpha = 1,position = "jitter")+
  scale_color_brewer(palette = "Paired")+
  #facet_wrap(~ Plot)+
  labs(y="Corrected weight of interlinks",x=NULL,color = NULL)+
  coord_flip()+
  theme_bw()+
  theme(axis.text.y = element_text(face = "italic"))
p1+p2
...
png("../New_Figures/figA41.png", width=1476*2, height = 1476*2*600/513, res=300*2)
p1/p2+ plot_layout(guides = "collect")
dev.off()


ggplot(interlinks_list_real %>% filter(weight<=1),aes(x=weight, y = weight*weight2, color = as.factor(G_F)))+
  geom_point(alpha = 1, position = "jitter")+
  geom_abline(slope = 1,intercept = 0, linetype = "dashed")+
  scale_color_brewer(palette = "Paired")+
  facet_wrap(~ Plot+layer_from)+
  #geom_smooth(method = "lm", se = F)+
  labs(y="Corrected weight of interlinks",x="Phenological weight of interlinks",
       color = NULL)+
  theme_bw()


ggplot(interlinks_list_real %>% filter(layer_from %in% c("C. fuscatum","L. maroccanus",
                                                         "P. paludosa")),
       aes(x=weight, y = weight*weight2, color = as.factor(G_F)))+
  geom_point(alpha = 1, position = "jitter")+
  geom_abline(slope = 1,intercept = 0, linetype = "dashed")+
  scale_color_brewer(palette = "Paired")+
  facet_wrap(~ Plot+layer_from)+
  #geom_smooth(method = "lm", se = F)+
  labs(y="Corrected weigth of interlinks",x="Phenological weigth of interlinks",
       color = NULL)+
  theme_bw()
...
ggplot(interlinks_list_real %>% filter(layer_from %in% c("C. fuscatum","L. maroccanus",
                                                         "P. paludosa")),
       aes(x=weight, y = weight*weight2, color = as.factor(G_F)))+
  geom_point(alpha = 1, position = "jitter",size = 2)+
  geom_abline(slope = 1,intercept = 0, linetype = "dashed")+
  scale_color_brewer(palette = "Paired")+
  facet_wrap(~ layer_from)+
  #geom_smooth(method = "lm", se = F)+
  labs(y="Corrected weigth of interlinks",x="Phenological weigth of interlinks",
       color = NULL)+
  theme_bw()

cor(interlinks_list$weight,interlinks_list$weight*interlinks_list$weight2,method = "spearman")
cor(interlinks_list$weight,interlinks_list$weight*interlinks_list$weight2)

for(i.plot in c(1,2,3,4,6,7,8,9)){
  print(i.plot)
  print(cor(interlinks_list$weight[interlinks_list$Plot == i.plot],
            interlinks_list$weight[interlinks_list$Plot == i.plot]*
              interlinks_list$weight2[interlinks_list$Plot == i.plot],
            method = "spearman"))
